			...
			<div className="header navbar navbar-static-top">
				<div className="container">
					...
					<ul className="nav navbar-nav nav-pills">
						<li className={page === '/start' ? 'choose' : ''}>
							<a href="#/start">起步</a>
						</li>
						<li className={page === '/css' ? 'choose' : ''}>
							<a href="#/css">全局CSS样式</a>
						</li>
						<li className={page === '/component' ? 'choose' : ''}>
							<a href="#/component">组件</a>
						</li>
						<li className={page === '/javascript' ? 'choose' : ''}>
							<a href="#/javascript">Javascript插件</a>
						</li>
						<li className={page === '/maker' ? 'choose' : ''}>
							<a href="#/maker">定制</a>
						</li>
					</ul>
				</div>
...
var AsidePart = React.createClass({
	...
	createList: function() {
		return this.props.data.map(function (obj, index) {
			return (
				<li key={index}>
					<a onClick={this.changePosition.bind(this, obj.id)}>{obj.title}</a>
				</li>
			)
		}.bind(this))
	},
...
// 想用Page组件复用所有页面成本太高，因为我们要传递很多参数，所以我们要将每一个页面定义成一个组件
var StartPage = React.createClass({
	render: function () {
		return (
			<div>
				<Page title="起步" intro="简要介绍 Bootstrap，以及如何下载、使用，还有基本模版和案例，等等。" url="data/start.json"></Page>
			</div>
		)
	}
})
var CssPage = React.createClass({
	render: function () {
		return (
			<div>
				<Page title="全局 CSS 样式" intro="设置全局 CSS 样式；基本的 HTML 元素均可以通过 class 设置样式并得到增强效果；还有先进的栅格系统。" url="data/css.json"></Page>
			</div>
		)
	}
})
var ComponentPage = React.createClass({
	render: function () {
		return (
			<div>
				<Page title="组件" intro="无数可复用的组件，包括字体图标、下拉菜单、导航、警告框、弹出框等更多功能。" url="data/component.json"></Page>
			</div>
		)
	}
})
var JavascriptPage = React.createClass({
	render: function () {
		return (
			<div>
				<Page title="JavaScript 插件" intro="jQuery 插件为 Bootstrap 的组件赋予了“生命”。可以简单地一次性引入所有插件，或者逐个引入到你的页面中。" url="data/js.json"></Page>
			</div>
		)
	}
})
var MakerPage = React.createClass({
	render: function () {
		return (
			<div>
				<Page title="定制并下载 Bootstrap" intro="通过自定义 Bootstrap 组件、Less 变量和 jQuery 插件，定制一份属于你自己的 Bootstrap 版本吧。" url="data/download.json"></Page>
			</div>
		)
	}
})
...
var routes = (
	<Route path="/" handler={App}>
		<Route path="start" handler={StartPage}></Route>
		<Route path="css" handler={CssPage}></Route>
		<Route path="component" handler={ComponentPage}></Route>
		<Route path="javascript" handler={JavascriptPage}></Route>
		<Route path="maker" handler={MakerPage}></Route>
...
// 用React渲染页面
// React.render(<App showPage="def" />, document.getElementById('app'))